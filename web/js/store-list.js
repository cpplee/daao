...
{	
	...
	$("#live-date").val(livedate);
	$("#leave-date").val(leavedate);
...
{	
	...
	{	
		...
		if(data.total === 0)
		{
			$("#hotellist").append("<p>对不起，您选择的地方没有专门店</p>");
		}
	...
	function showData(data,showCount)
	{
		...
		{
			if(data.data[i].storeLocation == null)
			{
				AddressDes = "专门店未添加地点描述信息"; 
			}
			else
			{
				AddressDes = data.data[i].storeLocation;
			}
			if(data.data[i].storeImgUrl == null)
			{
				ImgUrl = Rootpath + "/img/empty-sm.jpg";
			}
			else
			{
				ImgUrl = Rootpath + data.data[i].storeImgUrl;
			}
			...
			$("#page").before('<div class="block-rooms"><div class="row-fluid">'+
					...
					'title="'+data.data[i].storeName+'"><i class="icon-fullscreen"></i></a>'+
					'</div><img id="hotelImg" src="'+ImgUrl+'" class="img-notfound" alt="专门店图片未上传" style="height:133px;"></div></div>'+
					'<div class="span6"><a href="store-detail.html?storeid='+data.data[i].id+'" class="room-title">'+data.data[i].storeName+'</a>'+
					 '<div class="stars pull-right"><span class="star"></span><span class="star"></span><span'+
					'class="star"></span><span class="star"></span><span'+
					'class="star"></span></div><div class="room-price"><span></span>'+data.data[i].storePrice+'元<span> 起</span></div><p>'+AddressDes
					+'</p></div></div>');		
		}
		
	}
	
	var operation  = new Operation("前台展示.酒店信息.列专门店信息");
