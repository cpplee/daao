...
{
   var date = new Date;
	$("#eat-date").val(dateToStr1(date));
	$("#end-date").val(dateToStr1(date));
...
function onBook()
{
	...
	{
	$("#search-btn").text("搜索中...");
	window.location.href = "restaurant-list.html?destination="+$("#homecity_name").val()+"&livedate="+$("#eat-date").val()+"&leavedate="+$("#end-date").val();
	}
	else
	{
		alert("请选择您要订餐的地点");
