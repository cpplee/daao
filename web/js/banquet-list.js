...
{	
	...
	{	
		...
		if(data.total === 0)
		{
			$("#hotellist").append("<p>对不起，您选择的地方没有宴会</p>");
		}
	...
	function showData(data,showCount)
	{
		...
		{
			//alert(data.data[i].banName);
			if(data.data[i].banLocation == null)
			{
				AddressDes = "宴会未添加地点描述信息"; 
			}
			else
			{
				AddressDes = data.data[i].banLocation;
				
			}
			if(data.data[i].banImgUrl == null)
			{
				ImgUrl = Rootpath + "/img/empty-sm.jpg";
			}
			else
			{
				ImgUrl = Rootpath + data.data[i].banImgUrl;
			}
			...
			$("#page").before('<div class="block-rooms"><div class="row-fluid">'+
					...
					'title="'+data.data[i].banName+'"><i class="icon-fullscreen"></i></a>'+
					'</div><img id="hotelImg" src="'+ImgUrl+'" class="img-notfound" alt="宴会图片未上传" style="height:133px;"></div></div>'+
					'<div class="span6"><a href="banquest-detail.html?banquestId='+data.data[i].id+'&livedate='+livedate+'&leavedate='+leavedate+'" class="room-title">'+data.data[i].banName+'</a>'+
					 '<div class="stars pull-right"><span class="star"></span><span class="star"></span><span'+
					'class="star"></span><span class="star"></span><span'+
					'class="star"></span></div><div class="room-price"><span></span>'+data.data[i].banqPrice+'元<span> 起</span></div><p>'+AddressDes
					+'</p></div></div>');		
		}
		
	}
	
	var operation  = new Operation("前台展示.酒店信息.列宴会信息");
